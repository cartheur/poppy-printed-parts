...
#define _CMD_DZ_TYPE    "config_descriptor"
#define _CMD_DEBUG      "set_debug"

#define _NUM_OF_CMD 8

//available  commands
char * keyworld [] = {_CMD_HELP, _CMD_CLEAR, _CMD_BACKLIGHT, _CMD_DISPLAY, _CMD_DIGITIZER, _CMD_COORDINATE, _CMD_DZ_TYPE, _CMD_DEBUG};
...
int execute (int argc, const char * const * argv){
	...
	while (i < argc) {
		if (strcmp (argv[i], _CMD_HELP) == 0) {
            ...
            printf ("\tconfig_xy           - Set the config of the digitizer <0..6>\n");
            printf ("\tconfig_descriptor   - Set HID descriptor <0..3> (requires restart)\n");
            ...
            if (++i < argc) {
		        ...
		        if(val >= 0 && val <= 255){
					...
					printf("Wrong value '%s'. Not in <0..255>\n", argv[i]);
				    return 1;
                }
            }
            else {
            ...
            if (++i < argc) {
		        ...
		        if(val >= 0 && val <= 6){
					...
					printf("Wrong value '%s'. Not in <0..6>\n", argv[i]);
				    return 1;
                }
            }
            else {
                printf ("Current config: %d\n", digitizer_get_config());
			    printf ("specify value, <0..6>\n");
				return 1;
			}
        }
        else if(strcmp(argv[i], _CMD_DZ_TYPE) == 0){
            if (++i < argc) {
                int val = atoi (argv[i]);
		        if(val >= 0 && val <= 6){
                    set_descriptor_config(val);
                    write_descriptor_config(val); // Update EEPROM
                    printf ("HID descriptor updated to type %i\n", val);
                }
                else {
					printf("Wrong value '%s'. Not in <0..3>\n", argv[i]);
				    return 1;
                }
            }
            else {
			    printf ("specify value, <0..3>\n");
				return 1;
			}
        }
