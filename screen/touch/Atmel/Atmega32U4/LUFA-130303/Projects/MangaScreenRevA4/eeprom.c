...
uint8_t EEMEM ee_config_descriptor = 0;  // First time booting, Touch screen, normal
...
// Read the digitizer config type. 
int read_descriptor_config(void){
    return eeprom_read_byte (&ee_config_descriptor);
}

// Write the digitizer config type
void write_descriptor_config(int config){
    eeprom_update_byte (&ee_config_descriptor, config);
}
