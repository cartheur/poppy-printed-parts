...
void SetupHardware(void){
	...
	BL_on(0);
	_delay_ms(4); // Wait at least 4ms before enabling the output. Not sure why..
    ...
    if(ret){
        ...
        int config = read_digitizer_config();
        ret = digitizer_set_config(config);
	    dev_err("Digitizer OK: %x\n", ret);

        config = read_descriptor_config();
        ret = set_descriptor_config(config);
	    dev_err("Descriptor config OK: %x\n", ret);
   }
...
void dev_dbg(const char* format, ...){
    va_list args;
...
void dev_info(const char* format, ...){
    va_list args;
    va_start(args, format);
    if(debug_level <= 20){
        vprintf(format, args);
...
void dev_warn(const char* format, ...){
    va_list args;
    va_start(args, format);
    if(debug_level <= 30){
        vprintf(format, args);
...
void dev_err(const char* format, ...){
    va_list args;
    va_start(args, format);
    if(debug_level <= 40){
        vprintf(format, args);
...
bool CALLBACK_HID_Device_CreateHIDReport(USB_ClassInfo_HID_Device_t* const HIDInterfaceInfo,
	...
	USB_DigitizerReport_Data_t* report = (USB_DigitizerReport_Data_t*)ReportData;

    if(digitizer_get_report(report) == 0)
