...
static int digitizer_config_xy = CONFIG_DIGITIZER_SWITCH_INV_Y;
...
int digitizer_init(void){
	...
	if (error) {
		dev_err("ts probe failed, errno: %d\n", error);
		return error;
	}
...
/* Set the digitizer configuration */
int digitizer_get_config(){
    return digitizer_config_xy;
}
...
int digitizer_get_report(USB_DigitizerReport_Data_t* report){	
    ...
    if(tp[report_nr].reported == 0){
        ...
        report->Tip_and_InRange       = (tp[report_nr].event & TOUCH_EVENT_ON) ? 0x31 : 0;
        ...
        //dev_dbg("X %4d, Y: %4d, config: %d\n", report->X, report->Y, digitizer_config_xy);
